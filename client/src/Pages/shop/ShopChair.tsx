...
const ShopChair = () => {
    ...
    const [chair, setProductsChair] = useState<any[]>([]);
    const fetchProducts = async () => {
        let responseChair = await fetch(
            `http://localhost:8000/api/v1/products?category=2`
        );
        let dataChair = await responseChair.json();

        setProductsChair(dataChair);
    };
    ...
    useEffect(() => {
        const sortedLoudspeakerProducts = sortProducts(chair, sortOrder);
        setProductsChair(sortedLoudspeakerProducts);
    }, [sortOrder]);
